...
const createBooking = async (payload) => {
    ...
    const startDateInMS = new Date(rent_start_date).getTime();
    const endDateInMS = new Date(rent_end_date).getTime();
    ...
    `, [vehicle_id]);
        ...
        const isVehicleAvailable = availability_status === "available" ? true : false;
...
const updateBooking = async (id, role, status) => {
    const client = await db_1.pool.connect();
    try {
        await client.query("BEGIN");
